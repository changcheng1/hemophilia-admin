...
const fileTypeLabel: Record<string, string> = {
  ...
  hardship_material: '困难材料',
}
const detailFiles = computed(() =>
  (current.value?.files || []).map((file: any) => {
...
const previewImageUrls = computed(() => detailFiles.value.filter((file: any) => file.isImage && file.url).map((file: any) => file.url))
const regularDetailFiles = computed(() => detailFiles.value.filter((file: any) => file.fileType !== 'hardship_material'))
const hardshipMaterialFiles = computed(() => detailFiles.value.filter((file: any) => file.fileType === 'hardship_material'))
const threeElementVerificationText = computed(() => (current.value ? getThreeElementVerificationText(current.value) : '-'))
...
const resetEnrollment = async (row: EnrollmentListItem) => {
  try {
    await ElMessageBox.confirm('重置入组状态后，用户需要重新确认并提交入组信息，确认重置？', '重置入组状态', { type: 'warning', confirmButtonText: '确认', cancelButtonText: '取消' })
  } catch (error) {
...
const submitReview = async () => {
  ...
  await ElMessageBox.confirm(reviewResult.value === 'approve' ? '确认审核通过该入组资料？' : '确认驳回该入组资料？', '审核确认', { type: 'warning' })
  ...
  <main class="enrollment-management">
    <el-card class="search-card">
      <el-form :model="query" class="search-form">
        <el-row :gutter="20">
          <el-col :span="6"
            ><el-form-item label="援助项目"><el-input v-model="query.donationProject" placeholder="请输入" clearable /></el-form-item
          ></el-col>
          <el-col :span="6"
            ><el-form-item label="手机号码"><el-input v-model="query.phone" placeholder="请输入" clearable /></el-form-item
          ></el-col>
          <el-col :span="6"
            ><el-form-item label="患者姓名"><el-input v-model="query.recipientName" placeholder="请输入" clearable /></el-form-item
          ></el-col>
          <el-col :span="6"
            ><el-form-item label="证件号码"><el-input v-model="query.idNumber" placeholder="请输入" clearable /></el-form-item
          ></el-col>
        </el-row>
        <el-row :gutter="20">
          <el-col :span="6"
            ><el-form-item label="状态"
              ><el-select v-model="query.status" placeholder="请选择" clearable style="width: 100%"><el-option label="待审核" value="pending_review" /><el-option label="审核通过" value="approved" /><el-option label="初审驳回" value="rejected" /><el-option label="待重新提交" value="resubmit_required" /></el-select></el-form-item
          ></el-col>
          <el-col :span="12"
            ><el-form-item label="申请日期"><el-date-picker v-model="query.dateRange" type="daterange" value-format="YYYY-MM-DD" range-separator="～" start-placeholder="开始日期" end-placeholder="结束日期" style="width: 100%" /></el-form-item
          ></el-col>
        </el-row>
        <el-row
          ><el-col :span="24" class="search-buttons"><el-button type="primary" @click="search">搜索</el-button><el-button @click="reset">重置</el-button></el-col></el-row
        >
      </el-form>
    </el-card>
    ...
    <el-card class="list-card">
      <template #header
        ><div class="card-header"><span>入组列表</span><el-button type="primary" @click="showProjectReset">重置入组状态</el-button></div></template
      >
      <el-table :data="rows" v-loading="loading" stripe style="width: 100%" class="enrollment-table">
        ...
        <el-table-column label="手机号" min-width="120"
          ><template #default="{ row }">{{ row.user?.phone || '-' }}</template></el-table-column
        >
        ...
        <el-table-column label="申请状态" min-width="110"
          ><template #default="{ row }"
            ><el-tag :type="statusType[row.status]">{{ statusLabel[row.status] || row.status }}</el-tag></template
          ></el-table-column
        >
        <el-table-column label="申请时间" min-width="170"
          ><template #default="{ row }">{{ formatBusinessDateTime(row.submittedAt || row.createdAt) }}</template></el-table-column
        >
        <el-table-column label="操作" width="270" fixed="right"
          ><template #default="{ row }"
            ><div class="action-buttons"><el-button type="info" size="small" @click="showDetail(row)">查看</el-button><el-button v-if="row.status === 'pending_review'" type="primary" size="small" @click="showReview(row)">审核</el-button><el-button v-if="row.status === 'approved' || row.status === 'rejected'" type="warning" size="small" @click="resetEnrollment(row)">重置入组状态</el-button></div></template
          ></el-table-column
        >
      </el-table>
      <div class="pagination-container">
        <el-pagination v-model:current-page="query.page" v-model:page-size="query.limit" :page-sizes="[10, 20, 50, 100]" :total="total" layout="total, sizes, prev, pager, next, jumper" @size-change="handlePageSize" @current-change="handlePage" />
      </div>
    </el-card>
    ...
    <el-dialog v-model="projectResetVisible" title="重置入组状态" width="430px">
      <el-form label-width="64px" @submit.prevent>
        <el-form-item label="项目" required>
          <el-select v-model="resetProjectId" placeholder="请选择" filterable :loading="projectsLoading" style="width: 100%">
      ...
      <div v-if="current" class="enrollment-detail">
        <section class="detail-section">
          ...
          <el-descriptions :column="2" border>
            ...
            <el-descriptions-item label="状态"
              ><el-tag :type="statusType[current.status]">{{ statusLabel[current.status] || current.status }}</el-tag></el-descriptions-item
            >
        ...
        <section class="detail-section">
          ...
          <el-descriptions :column="2" border>
            ...
            <el-descriptions-item label="三要素验证" :span="2"
              ><span class="verification-result">{{ threeElementVerificationText }}</span></el-descriptions-item
            >
        ...
        <section class="detail-section">
          <h3>上传材料</h3>
          <el-empty v-if="!regularDetailFiles.length" description="暂无其他上传材料" :image-size="72" />
          <div v-else class="file-grid">
            <article v-for="file in regularDetailFiles" :key="file.id" class="file-card">
              <div class="file-label">{{ file.label }}</div>
              <el-image v-if="file.isImage && file.url" :src="file.thumbnailUrl" :preview-src-list="previewImageUrls" :initial-index="previewImageUrls.indexOf(file.url)" fit="cover" class="file-image" lazy preview-teleported>
        ...
        <section class="detail-section hardship-material-section">
          <h3>困难材料</h3>
          <el-empty v-if="!hardshipMaterialFiles.length" description="暂无困难材料" :image-size="72" />
          <div v-else class="file-grid">
            <article v-for="file in hardshipMaterialFiles" :key="file.id" class="file-card">
              <div class="file-label">{{ file.label }}</div>
              <el-image v-if="file.isImage && file.url" :src="file.thumbnailUrl" :preview-src-list="previewImageUrls" :initial-index="previewImageUrls.indexOf(file.url)" fit="cover" class="file-image" lazy preview-teleported>
                <template #error><div class="file-image-error">图片加载失败</div></template>
              </el-image>
